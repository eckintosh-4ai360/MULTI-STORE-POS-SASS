      ...
      <div className="glass-modal rounded-3xl w-full max-w-sm overflow-hidden animate-scaleIn">
        <div className="p-6 text-center border-b border-white/20">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Change: <span className="font-semibold text-emerald-600">GHS {sale.change.toFixed(2)}</span>
          </p>
        </div>
        <div className="p-5 font-mono text-xs space-y-1 bg-gray-50/80">
          ...
          {sale.discountAmount > 0 && (
            <div className="flex justify-between text-red-500">
              <span>Discount</span>
              <span>-GHS {sale.discountAmount.toFixed(2)}</span>
            </div>
          )}
          <div className="flex justify-between text-gray-600">
            <span>Tax ({store?.taxRate}%)</span>
            <span>GHS {sale.taxAmount.toFixed(2)}</span>
          </div>
          ...
          <div className="flex justify-between text-gray-500">
            ...
            <span>GHS {sale.amountPaid.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-gray-500">
            <span>Change</span>
            <span>GHS {sale.change.toFixed(2)}</span>
          </div>
          <div className="border-t border-dashed border-gray-300 my-2" />
