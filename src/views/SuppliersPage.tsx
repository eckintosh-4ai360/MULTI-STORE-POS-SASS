    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
          <button onClick={() => setActiveTab("suppliers")} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${activeTab === "suppliers" ? "bg-white shadow text-gray-800" : "text-gray-500"}`}>Suppliers</button>
          <button onClick={() => setActiveTab("orders")} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${activeTab === "orders" ? "bg-white shadow text-gray-800" : "text-gray-500"}`}>
      ...
      {activeTab === "suppliers" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {storeSuppliers.map(sup => (
            <div key={sup.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition">
              <div className="flex items-start justify-between mb-3">
                ...
                <button onClick={() => openEdit(sup)} className="p-1.5 text-gray-300 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition"><Edit2 size={14} /></button>
              </div>
              <h3 className="font-semibold text-gray-800">{sup.name}</h3>
              <div className="mt-2 space-y-1">
                <div className="flex items-center gap-1.5 text-xs text-gray-500"><Phone size={11} />{sup.phone}</div>
                {sup.email && <div className="flex items-center gap-1.5 text-xs text-gray-500"><Mail size={11} />{sup.email}</div>}
                {sup.address && <div className="flex items-center gap-1.5 text-xs text-gray-500"><Truck size={11} />{sup.address}</div>}
              </div>
              {sup.balance > 0 && (
            ...
            <tbody className="divide-y divide-gray-50">
              {storeOrders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(order => (
                <tr key={order.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 font-semibold text-gray-800">GHS {order.total.toFixed(2)}</td>
                  <td className="px-4 py-3 text-xs text-gray-500">{format(new Date(order.createdAt), "dd MMM yyyy")}</td>
                  ...
                  <td className="px-4 py-3">
                    {order.status === "pending" && (
                      <button onClick={() => receivePurchaseOrder(order.id)} className="flex items-center gap-1 text-xs text-emerald-600 hover:bg-emerald-50 px-2 py-1 rounded-lg transition">
